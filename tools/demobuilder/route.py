...
class Autorouter:
    ...
    def get_arcs_downhill(self, wire):
        if wire in self.dh_arc_cache:
            ...
            try:
                npos = tiles.pos_from_name(wire, self.chip.info.family, chip_size, self.row_bias, self.col_bias)
            ...
            for pos in positions:
                for tile in self.chip.get_tiles_by_position(pos[0], pos[1]):
                    ...
                    for sink in downhill:
                        nn = nets.canonicalise_name(self.chip.info.family, self.chip_size, tname, sink.first, self.row_bias, self.col_bias)
                        if nn is not None:
    ...
    def route_net_to_wire(self, net, wire, config):
        ...
        chip_size = (self.chip.get_max_row(), self.chip.get_max_col())
        dest_pos = tiles.pos_from_name(wire, self.chip.info.family, chip_size, self.row_bias, self.col_bias)
        def get_score(x_wire):
            pos = tiles.pos_from_name(x_wire, self.chip.info.family, chip_size, self.row_bias, self.col_bias)
